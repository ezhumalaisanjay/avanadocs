# Multi Filter Distributor

### API Overview
- **Resource Name:** `multi_filter_distributor`
- **Sub Resource Name:** `multi_filter_distributor`
- **Method:** `PUT`
- **Invoke URL:** `https://xqaizmksl2.execute-api.us-west-2.amazonaws.com/test/multi_filter_distributor`
- **Lambda Function:** ``
...
def lambda_handler(event, context):
    ...
    if 'distributor_names' in event and isinstance(event['distributor_names'], list):
        distributor_names = event['distributor_names']
    else:
        return {
            'statusCode': 400,
    ...
    for distributor_name in distributor_names:
        ...
        while True:
            ...
            query_params = {
                'KeyConditionExpression': current_filter_expressions[0]
            }
            if len(current_filter_expressions) > 1:
                query_params['FilterExpression'] = reduce(lambda x, y: x & y, current_filter_expressions[1:])
    ...
    matching_records = prepare_response_data(all_items)
...
def prepare_response_data(items):
    ...
    for item in items:
        ...
        key = (distributor_name, doctors_name)
        ...
        consolidated_records[key]['doctor_details'].append({
